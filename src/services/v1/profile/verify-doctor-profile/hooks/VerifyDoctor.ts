...
import { User_GET, UserRole, UserStatus } from '../../../../../db_services/v1/user/interfaces/UserInterfaces';
...
const VerifyDoctor = () => async (context: HookContext) => {
    ...
    const userResponse = await app
        .service(userPath)
        ._patch(id.toString(), { status: UserStatus.ACTIVE }, { query: { role: UserRole.DOCTOR } })
    ...
    if (userData.doctorProfile) {
        ...
        await app.service(doctorProfilePath)._patch(userData.doctorProfile.toString(), {
            status: EntityStatus.ACTIVE,
        });
