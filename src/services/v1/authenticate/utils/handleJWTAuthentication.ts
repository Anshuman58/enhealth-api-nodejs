...
const handleJWTAuthentication = async (context: HookContext): Promise<any> => {
    ...
    if (!params.headers || !params.headers.authorization) throw new NotAuthenticated('AccessToken is required in header!');
    ...
    if (exp) {
        if (exp * 1000 > Date.now()) {
            ...
            const { accessToken, user } = await AuthHelper.generateAccessToken(userData, true);
            response = {
                accessToken,
                user: user,
            };
