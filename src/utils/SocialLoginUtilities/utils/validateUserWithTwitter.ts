...
const validateUserWithTwitter = async (
    ...
    const signatureBaseString = new oauthSignature.SignatureBaseString(method, twitterOauthUrl, parameters).generate();
    const signingKey = twitterOauthConsumerSecret + '&' + twitterOauthTokenSecret;
    let twitterOauthSignature = crypto.createHmac('sha1', signingKey).update(signatureBaseString).digest('base64');
    ...
    if (!email) {
        return {
            ...
            message: 'Please provide email access to the application while authorizing with twitter.',
        };
    }
